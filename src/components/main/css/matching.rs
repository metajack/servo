...
impl<'ln> MatchMethods for LayoutNode<'ln> {
    ...
    fn cascade_subtree(&self, parent: Option<LayoutNode>) {
        macro_rules! cascade_node(
            ($applicable_declarations: ident, $style: ident) => {{
                ...
                let computed_values = {
                    ...
                    Arc::new(cascade(layout_data.$applicable_declarations, parent_style.map(|parent_style| parent_style.get())))
                };
...

        {
            let before_len = {
                let layout_data_ref = self.borrow_layout_data();
                layout_data_ref.get().as_ref().unwrap().before_applicable_declarations.len()
            };
            if before_len > 0 {
        ...
        cascade_node!(applicable_declarations, style);
        {
            let after_len = {
                let layout_data_ref = self.borrow_layout_data();
                layout_data_ref.get().as_ref().unwrap().after_applicable_declarations.len()
            };
            if after_len > 0 {
